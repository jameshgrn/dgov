...
| **FIXED** ~~`src/dgov/blame.py:181`, `src/dgov/blame.py:306`~~ | medium | ~~`blame_lines()` can call `_slug_from_sha_subject()` once per line, creating O(n) subprocesses.~~ | SHA resolution memoized via `_slug_cache` dict inside `blame_lines()`. |
