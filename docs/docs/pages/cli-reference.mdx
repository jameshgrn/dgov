...
Every `dgov` command and flag, grouped by what you do with them.
...
Stream events in real time. Useful in a second terminal during a run.
...
Operational ledger — record and query bugs, rules, patterns, decisions, and
debt. Backed by SQLite alongside the event log.
...
Categories: `bug`, `rule`, `pattern`, `decision`, `debt`.
...
spawning a worker. Useful for "does my checkout pass the same bar a task
would have to clear?"
...
heuristics. Use this to triage what to refactor next.
