...
`in progress`, `complete`, `stale`, or `degraded`. `stale` means the compiled
artifact is older than the source TOMLs; `degraded` means every unit deployed
but a final-stage gate reported degradation. JSON mode emits the full per-plan
payload, including `run_status` and `remediation_needed`.
