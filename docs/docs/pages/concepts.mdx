# Concepts

`dgov` is a local governor for AI coding work. It is built around one idea:
the orchestration layer should behave more like a compiler than like an
autonomous assistant.

That shapes almost every design decision in the project.

## Philosophy

### Determinism Over Vibes

The worker can be probabilistic. The governor should not be.

`dgov` treats planning, task ordering, retry policy, file claims, settlement,
and merge decisions as explicit state transitions. The orchestration logic is
meant to be inspectable and reproducible, even when the model itself is not.

### Isolation Over Shared Context

Each task runs in its own git worktree. That is not an implementation detail.
It is the main safety boundary.

The benefit is straightforward:

- concurrent tasks do not scribble over the same checkout
- every attempt has a concrete filesystem snapshot
- failed work can be rejected without contaminating main

### Validation Over Trust

`dgov` does not assume the worker is correct because it sounds confident.
Changes are reviewed and validated before merge:

- scope checks
- formatter and lint fixes
- format and lint gates
- targeted tests
- sentrux architectural checks

This is why the system can be stricter than a chat assistant while still being
useful.

The sentrux baseline is explicit governor-owned state. Workers are judged
against `.sentrux/baseline.json`; they do not get to rewrite it as part of a
task.

### Explicit Claims Over Implicit Intent

Every task must declare the files it touches. That is a constraint, not
boilerplate.

Without file claims, parallel execution turns into guesswork. With claims,
`dgov` can reject invalid plans before running them and can explain why two
tasks cannot safely run independently.

## What dgov Is

`dgov` is:

- a DAG-based execution engine for AI coding tasks
- a local git-worktree orchestrator
- a settlement layer around model output
- a tool for reproducible, inspectable automation in real repos

`dgov` is not:

- a replacement for git
- a remote task queue
- a multi-provider abstraction framework
- a general-purpose chat frontend
- a system that hides the underlying repo state from you

If you want “just do something smart in this checkout,” there are simpler
tools. `dgov` exists for cases where the structure and auditability matter.

## Architecture

| Module | Role |
|--------|------|
| `kernel.py` | Pure `(state, event) -> (new_state, actions)` transition logic |
| `runner.py` | Async bridge from kernel actions to real I/O |
| `worker.py` | OpenAI-compatible worker subprocess |
| `worktree.py` | Snapshot isolation via git worktrees |
| `settlement.py` | Review, autofix, validation, and sentrux gates |
| `plan_tree.py` | Tree walk, merge, ref resolution, and validation |
| `plan.py` | Compiled plan parsing and DAG compilation |
| `persistence/` | SQLite event log, runtime artifact cache, and slug history |

## Kernel Model

The kernel is the center of the system. It does not know about subprocesses,
git, HTTP, or the OpenAI client. It only knows:

- event-derived lifecycle state
- incoming events
- outgoing actions

That separation matters because it gives you:

- strong unit-testability
- crash recovery through event replay
- clearer failure boundaries
- fewer “hidden” side effects in orchestration code

The runner does the messy part. The kernel stays small and explicit.

## Authorities

Three stores matter, and they do different jobs:

- **Event log**: lifecycle authority. Dispatch, retry, status, watch, and live cleanup decisions derive from events.
- **Deploy log**: landed-output authority. This answers what actually made it onto main.
- **Runtime artifact rows**: bookkeeping only. These cache worktree paths, branch names, and similar operational crumbs for debugging and cleanup, but they do not define lifecycle truth.

## Snapshot Isolation

Every dispatched task gets a separate branch and worktree rooted at a specific
commit. That gives `dgov` a concrete notion of “attempt.”

This is why `dgov` historically required `HEAD`, and why the bootstrap-commit
behavior is careful: the system needs a real snapshot to branch from. It does
not need GitHub, but it does need local git state.

## Settlement

The worker is the implementer. Settlement is the auditor.

The sequence is roughly:

1. fast review checks
2. mechanical cleanup
3. validation and architectural gates
4. merge or reject

This is a deliberate split of responsibilities:

- workers edit
- settlement judges
- the governor decides retries and lifecycle

If you collapse those roles, you get a system that is harder to reason about
and much easier to fool.

## Semantic Settlement

Semantic settlement goes beyond simple merge gates. A clean `git merge` is
necessary but not sufficient for safe integration. The semantic layer
analyzes *what* changed, not just *that* something changed.

### Core Philosophy

Traditional merge gates ask: "Does this apply cleanly?" Semantic settlement
asks: "Will this work correctly when integrated with the current target?"

This distinction matters when:

- Two tasks touch the same Python symbol in different ways
- A function signature changes in one task while another task calls it
- Identical symbols get defined in separate files that both land
- Valid changes combine to produce invalid ordering or behavior

The semantic layer detects these integration risks *before* the merge
completes, using evidence derived from AST analysis, diff inspection, and
symbol-level overlap detection.

### Failure Taxonomy (`FailureClass`)

When semantic settlement rejects a candidate, it classifies the failure
precisely:

| Failure Class | Meaning |
|--------------|---------|
| `TEXT_CONFLICT` | Git cannot replay the task commit cleanly on target HEAD |
| `SYNTAX_CONFLICT` | The integrated file no longer parses (invalid Python/JSON/etc) |
| `SAME_SYMBOL_EDIT` | Both sides changed the same Python symbol (function, class, etc) |
| `DUPLICATE_DEFINITION` | Integrated code defines the same symbol in multiple files |
| `SIGNATURE_DRIFT` | A public callable changed its signature relative to task base or target |
| `ORDERING_CONFLICT` | Operations are valid individually but invalid in integrated order |
| `BEHAVIORAL_MISMATCH` | Parse-level checks pass but settlement gates (tests, lint, etc) fail |

Each failure class corresponds to a specific integration risk. The taxonomy
enables targeted remediation: a `SAME_SYMBOL_EDIT` suggests coordination
between tasks, while `SIGNATURE_DRIFT` suggests a stale task base.

### Risk Levels (`RiskLevel`)

Before attempting integration, `dgov` scores the *risk* of integration:

| Level | Meaning | Action |
|-------|---------|--------|
| `NONE` | No detectable risk | Standard settlement gates |
| `LOW` | Minimal risk | Standard settlement gates sufficient |
| `MEDIUM` | Elevated risk | Shadow-mode validation recommended |
| `HIGH` | Significant risk | Integrated candidate validation required |
| `CRITICAL` | Near-certain conflict | Reject without full integration attempt |

Risk scoring considers Python symbol overlap, file claim intersections, and
the recency of the task base relative to target HEAD. Critical risk tasks
are rejected early to avoid wasting compute on doomed integration attempts.

### Symbol Overlap Evidence (`SymbolOverlap`)

When two tasks touch the same code entity, `dgov` captures the overlap as
structured evidence:

- **symbol_name**: The affected function, class, or variable
- **symbol_type**: Classification (`function`, `class`, `method`, etc)
- **file_path**: Where the symbol lives
- **task_line_range**: Lines the task modified
- **target_line_range**: Lines the target has modified

This evidence enables precise conflict reporting. Instead of "merge failed,"
the system can report: "Task A and Task B both edited `process_order()` in
`checkout.py`--Task A at lines 45-52, Task B at lines 48-55."

### Integration Candidate Validation

The semantic layer builds ephemeral integration candidates to validate
merge outcomes before the real merge happens:

1. Create a temporary workspace rooted at target HEAD
2. Replay the task commit
3. Run the full settlement pipeline (syntax checks, tests, lint, sentrux)
4. Record pass/fail with specific failure class and evidence

Only candidates that pass this validation proceed to the actual merge. This
"shadow integration" catches problems that would otherwise appear only after
the merge was already recorded in git history.

## Plans As Control Surfaces

Plans are not just prompts in a file. They are the primary control surface for
execution:

- dependencies define legal order
- file claims define legal concurrency
- task prompts define local intent
- commit messages define merge outcomes

This is why `dgov compile` exists at all. The compile step turns a human-edited
plan tree into a normalized artifact that the runner can execute without
reinterpreting structure on the fly.

## Why Git Worktrees

Git worktrees are the core execution primitive because they give `dgov` three
things at once:

- cheap branch-isolated sandboxes
- straightforward merge semantics
- native inspectability with normal git tools

You can look at what happened with ordinary git commands instead of relying on a
hidden internal state format.

## Tradeoffs

`dgov` makes some tradeoffs on purpose.

### Stronger setup requirements

You need a real local repo, working toolchain commands, and an
OpenAI-compatible endpoint. That is more setup than a browser chat box.

The payoff is that the system operates on your real repo with real validation.

### More structure up front

Plans, file claims, and compile steps add ceremony. That ceremony buys you
safer concurrency, replayable execution, and fewer silent footguns.

### Narrower provider model

`dgov` supports OpenAI-compatible endpoints rather than every provider-native
API shape. That is a deliberate simplification. It keeps the runtime surface
small while still supporting Fireworks, OpenAI, OpenRouter, and similar APIs.

## Failure Model

A useful way to think about `dgov` is that it fails closed.

Examples:

- invalid plan references stop at compile time
- missing API key fails before dispatch
- out-of-scope edits fail review
- failing tests fail settlement
- rejected work is not merged

That behavior is not accidental. It is the point.

## Mental Model

The shortest accurate mental model is:

`dgov` is a deterministic governor around a probabilistic worker.

If that framing makes sense, the rest of the project tends to make sense too.
