...
@click.option("--project-root", "-r", default=".", envvar="DGOV_PROJECT_ROOT", help="Git repo root")
@SESSION_ROOT_OPTION
@click.option("--kind", "-k", default=None, help="Filter by decision kind (route_task, classify_output, review_output)")
...
def journal_cmd(project_root, session_root, kind, pane, limit, json_output):
    ...
    for row in rows:
        ...
        click.echo(f"{ts}  {k:<16s} {provider:<24s} {model:<16s} {slug:<20s} {dur_str:>8s}  {status}")
