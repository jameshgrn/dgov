...
def send_command(pane_id: str, command: str) -> None:
    ...
    # Minimal delay: tmux paste is synchronous but terminal needs ~10ms to render
    time.sleep(0.01)
    _run(["send-keys", "-t", pane_id, "Enter"])
    _run(["delete-buffer", "-b", buf_name], silent=True)
    # Brief settle for zsh to process Enter before next operation
    # Reduced from 200ms → 30ms as tmux operations are synchronous
    time.sleep(0.03)
...
def send_prompt_via_buffer(pane_id: str, prompt: str) -> None:
    ...
    _run(["paste-buffer", "-b", buf_name, "-t", pane_id])
    # Reduced from 500ms → 50ms: tmux paste is synchronous, terminal needs minimal time
    # This delay allows the shell to process input before Enter is sent
    time.sleep(0.05)
    _run(["send-keys", "-t", pane_id, "Enter"])
    _run(["delete-buffer", "-b", buf_name], silent=True)
...
# Session-aware tmux helpers for use outside tmux (e.g., runner)
def ensure_session(session_name: str) -> None:
    """Ensure a tmux session exists, creating it if necessary.

    Uses new-session -d to create a detached session.
...
def get_tmux_env_from_session(session_name: str) -> dict[str, str] | None:
    """Get TMUX env var for targeting a specific session from outside tmux.

    Returns a dict with TMUX key for subprocess env, or None if session not found.
    """
    result = subprocess.run(
    ...
    if result.returncode == 0:
        return {"TMUX": result.stdout.strip()}
    return None


def run_tmux_with_session(
    session_name: str | None,
    args: list[str],
    **kwargs,
) -> subprocess.CompletedProcess:
    """Run tmux command with proper session context for external callers.

    If session_name is provided, sets TMUX env var so commands target that session.
    Otherwise runs with current environment (assumes caller is inside tmux).
    """
    env = os.environ.copy()
    if session_name:
        session_env = get_tmux_env_from_session(session_name)
        if session_env:
...
def send_keys_with_session(
    pane_id: str,
    keys: str,
    session_name: str | None = None,
) -> subprocess.CompletedProcess:
    """Send keys to pane, optionally targeting a specific session."""
    return run_tmux_with_session(
        session_name,
        ["send-keys", "-t", pane_id, keys],
        capture_output=True,
        check=False,
    )
