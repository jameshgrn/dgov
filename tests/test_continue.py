...
async def _noop_model_env(self):
    pass
...
def _capture_current_scope_review(captured: dict[str, tuple[str, ...]]):
    ...
    return _review_current_scope


def _record_stale_task_row(session_root: str, git_repo: Path) -> None:
...
def _emit_stale_scope_failure(session_root: str) -> None:
    emit_event(
        session_root,
        "dag_task_dispatched",
        "pane-old",
...
def _current_claims_dag() -> DagDefinition:
    return _dag(
        {
            "t1": DagTaskSpec(
                slug="t1",
...
def test_continue_retries_failed_tasks(git_repo, monkeypatch):
    ...
    monkeypatch.setattr("dgov.runner.EventDagRunner._check_model_env", _noop_model_env)
...
def test_continue_uses_current_dag_claims_not_stale_task_rows(git_repo, monkeypatch):
    ...
    monkeypatch.setattr("dgov.runner.review_sandbox", _capture_current_scope_review(captured))

    session_root = str(git_repo)
    clear_connection_cache()

    _record_stale_task_row(session_root, git_repo)
    _emit_stale_scope_failure(session_root)
    dag = _current_claims_dag()
